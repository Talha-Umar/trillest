                        ...
                        <div class="card" style="padding: 10px 15px;">
                          <div class="row" style="padding: 5px 0px;border-bottom: 1px solid #000;">
                            ...
                            <div class="col-md-11" style="margin-top:12px;">
                                <strong id='name'>AK Sandy</strong>
...
/* Handle */
 ...
 <div id="GetMessages">
                    ...
                    $adminid=$_SESSION["admin_id"];
                     ...
                     $sender_id= $row['id'];
                     $sender_name= $row['name']." (admin)";
                     $sender_pic = '../admin/'.$row['profile'];
                      
                  
                      $gid=$_REQUEST['gymid'];
                      $sql2 = "SELECT * FROM gym WHERE id='$gid'";
                      $result2 = $con->query($sql2);
                      $row2 = $result2->fetch_assoc();
                      $gym_name= $row2['businessName'];
                      $gym_pic ='../gym/'.$row2['profile'];
...
<script>
    ...
    getcookiesdata();
        ...
        // save in database
        firebase.database().ref('gym').child('messages').child(ID).child('$sender_id').push().set({
         ...
         firebase.database().ref('user').child('messages').child('$sender_id').child(ID).push().set({
            'name': '$sender_name',
            'message': message,
            'participant': 'sender',
            'timestemp':time,
       ...
       firebase.database().ref('user').child('gymuser').child('$sender_id').child(ID).child('data').set({
            'name': NAME,
            'message': message,
            'timestemp':time,
            'dp':DP,
         ...
         firebase.database().ref('gym').child('user').child(ID).child('$sender_id').child('data').set({
            'message': message,
            'name':'$sender_name',
             'timestemp':time,
             'dp':'$sender_pic',
        });
         firebase.database().ref('gym').child('messagenotification').child(ID).child('$sender_id').push().set({
            'participant': 'receiver',
            'message': message,
            'name':'$sender_name',
             'timestemp':time, 
...
?>
  ...
  <script>
 if('<?php echo $gid ?>'!="")
    readmessage('<?php echo $gid ?>','<?php echo $gym_name ?>','<?php echo $gym_pic ?>')
    else
        document.getElementById('masghide').style.display='none';
  ...
  return cookie[cookieName];
         ...
         firebase.database().ref('user').child('gymuser').child('<?php echo $row['id'];?>').on('value',   function(snapshot) {
              $("#contacts").empty();
  ...
  function readdata(key){
     ...
     firebase.database().ref('user').child('gymuser').child('<?php echo $row['id'];?>').child(key).on('child_added',   function(snapshot) {
 ...
 firebase.database().ref("user").child("messages").on("child_added", function (snapshot) {
         ...
         firebase.database().ref('user').child('messages').child("<?php echo $sender_id; ?>").child(key).on('value',   function(snapshot) {
            $("#messagesuser").empty();
    snapshot.forEach(function(childSnapshot) {
